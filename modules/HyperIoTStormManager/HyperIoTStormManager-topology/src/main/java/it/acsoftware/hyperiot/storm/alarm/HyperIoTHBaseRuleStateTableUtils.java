...
public class HyperIoTHBaseRuleStateTableUtils {
    ...
    public static HashMap<Long,Boolean> retrieveRuleStateFromHBase (Map<String, Object> topologyConf, String tableName , String configKey) {
        ...
        try (HBaseClient hBaseClient = new HBaseClient(hbaseConfMap, hbConfig, tableName)) {
            ...
            for (Result result : scanner) {
                ...
                for (Map.Entry<byte[], byte[]> entry : resultFamilyMap.entrySet()) {
                    ...
                    projectEventRuleStateMap.put(firedRule.getRuleId(), firedRule.isFired());
                }
            }
            return projectEventRuleStateMap;
        } catch (Exception e) {
