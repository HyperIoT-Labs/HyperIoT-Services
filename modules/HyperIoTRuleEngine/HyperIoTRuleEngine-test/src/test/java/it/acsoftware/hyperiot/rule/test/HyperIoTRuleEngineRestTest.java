...
//		System.out.println(executeCommand("bundle:list | grep HyperIoT"));
    ...
    public void test02_saveRuleEngineShouldWork() {
        ...
        Assert.assertEquals(hpacket.getDevice().getProject().getId(), ((Rule) restResponse.getEntity()).getPacket().getDevice().getProject().getId());
        Assert.assertEquals(adminUser.getId(), ((Rule) restResponse.getEntity()).getPacket().getDevice().getProject().getUser().getId());
    ...
    public void test04_updateRuleEngineShouldWork() {
        ...
        Assert.assertNull(rule.getPacket());
...
//		engine.check(packet1);
    ...
    public void test35_saveRuleEngineShouldFailIfEntityIsDuplicated() {
        ...
        duplicateRule.setPacket(rule.getPacket());
    ...
    public void test36_updateRuleEngineShouldFailIfEntityIsDuplicated() {
        ...
        Assert.assertNull(duplicateRule.getPacket());
    ...
    public void test42_findAllRuleByPacketIdShouldWork() {
        ...
        Assert.assertEquals(adminUser.getId(), rule.getPacket().getDevice().getProject().getUser().getId());
        ...
        for (Rule r : listRulesByPacketId) {
            if (rule.getId() == r.getId()) {
                Assert.assertEquals(hproject.getId(), r.getProject().getId());
                Assert.assertEquals(adminUser.getId(), r.getProject().getUser().getId());
                Assert.assertEquals(hpacket.getId(), r.getPacket().getId());
                Assert.assertEquals(adminUser.getId(), r.getPacket().getDevice().getProject().getUser().getId());
                ruleFound = true;
            }
        }
        Assert.assertTrue(ruleFound);
