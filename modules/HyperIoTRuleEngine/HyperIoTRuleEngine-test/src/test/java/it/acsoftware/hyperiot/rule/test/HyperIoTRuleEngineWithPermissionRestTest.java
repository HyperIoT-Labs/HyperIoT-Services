...
//		System.out.println(executeCommand("bundle:list | grep HyperIoT"));
    ...
    public void test02_saveRuleWithPermissionShouldWork() {
        ...
        Assert.assertEquals(huser.getId(), ((Rule) restResponse.getEntity()).getPacket().getDevice().getProject().getUser().getId());
    }
    ...
    public void test47_findAllRuleByPacketIdWithPermissionShouldWork() {
        ...
        for (Rule r : listRulesByPacketId) {
            if (rule.getId() == r.getId()) {
                ...
                Assert.assertEquals(huser.getId(), r.getPacket().getDevice().getProject().getUser().getId());
                ruleFound = true;
            }
        }
        Assert.assertTrue(ruleFound);
    ...
    private Rule createRule(HProject hproject, HPacket hpacket) {
        ...
        if (hpacket != null) {
            Assert.assertEquals(hpacket.getDevice().getProject().getId(), ((Rule) restResponse.getEntity()).getPacket().getDevice().getProject().getId());
            Assert.assertEquals(ownerHUser.getId(), ((Rule) restResponse.getEntity()).getPacket().getDevice().getProject().getUser().getId());
        }
        removeDefaultPermission(ownerHUser);
        return rule;
    }
