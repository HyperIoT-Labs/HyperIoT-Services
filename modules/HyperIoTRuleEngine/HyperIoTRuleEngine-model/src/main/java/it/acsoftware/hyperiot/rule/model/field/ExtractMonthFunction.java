...
public class ExtractMonthFunction extends AbstractFieldFunction implements FieldFunction {
    //just one param
    RuleNode[] params = new RuleNode[1];
    @Override
    public void defineOperands(RuleNode... operands) {
        if (operands.length <= 0 || operands.length > 1)
            throw new HyperIoTRuntimeException("month function accepts only one param!");
    ...
    public String droolsDefinition() {
        return "LocalDate.ofInstant(Instant.ofEpochMilli((Long)" + this.params[0].droolsDefinition() + "), ZoneId.systemDefault()).getMonth()";
    }
}
