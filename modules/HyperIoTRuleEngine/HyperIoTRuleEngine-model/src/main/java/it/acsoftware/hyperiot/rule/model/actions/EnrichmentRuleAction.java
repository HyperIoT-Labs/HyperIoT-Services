...
public abstract class EnrichmentRuleAction extends RuleAction {
    ...
    protected String getDroolsPacketNameVariable() {
        if(this.getPacketIds() != null && this.getPacketIds().size() == 1){
            return "packet_"+this.getPacketIds().iterator().next();
        }
        return "packet_<not_available_or_too_may_packetIds>";
    }
}
