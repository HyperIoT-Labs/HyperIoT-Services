...
@Component(service = HyperIoTPreRemoveAction.class, property = {"type=it.acsoftware.hyperiot.hproject.algorithm.model.HProjectAlgorithm"})
public class HProjectAlgorithmPreRemoveAction <T extends HyperIoTBaseEntity> implements HyperIoTPreRemoveAction<T> {

    private static final Logger log = LoggerFactory.getLogger(HProjectAlgorithmPreRemoveAction.class.getName());
    ...
    public void execute(T entity) {
        long hProjectAlgorithmId = entity.getId();
        log.debug("Reset widget configuration related to HProjectAlgorithm with id {}", hProjectAlgorithmId);
        Collection<DashboardWidget> dashboardWidgets =  dashboardWidgetSystemApi.getAllDashboardWidgetByHProjectAlgorithmId(hProjectAlgorithmId);
