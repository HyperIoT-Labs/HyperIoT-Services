...
public class HyperIoTAlarmRestTest extends KarafTestSupport {
    ...
    @Test
    public void test009_saveAlarmShouldFailIfDuplicatedEntityException(){
        // hadmin tries to save Alarm with the following call saveAlarm,
        ...
        AuthenticationApi authService = getOsgiService(AuthenticationApi.class);
        HyperIoTUser adminUser = (HUser) authService.login("hadmin", "admin");
        Alarm alarm2 = createAlarmTemplate();
        alarm2.setName(alarm.getName());
        impersonateUser(alarmRestService, adminUser);
        Response restResponse = alarmRestService.saveAlarm(alarm2);
    ...
    @Test
    public void test020_updateAlarmShouldFailIfDuplicatedEntityException(){
        // hadmin tries to update Alarm with the following call saveAlarm,
        // but entity is duplicated
        // response status code '422' HyperIoTDuplicateEntityException
        Alarm alarm = createAlarm();
        Assert.assertNotEquals(0, alarm.getId());
        ...
        alarm2.setName(alarm.getName());
        impersonateUser(alarmRestService, adminUser);
        Response restResponse = alarmRestService.updateAlarm(alarm2);
        Assert.assertEquals(409, restResponse.getStatus());
        Assert.assertEquals(1, ((HyperIoTBaseError) restResponse.getEntity()).getErrorMessages().size());
        Assert.assertEquals("name", ((HyperIoTBaseError) restResponse.getEntity()).getErrorMessages().get(0));
    }
    ...
    @Test
    public void test034_saveAlarmAndEventsShouldFailIfDuplicatedEntityException(){
        // hadmin tries to save Alarm with the following call saveAlarm,
        // but entity is duplicated
        // response status code '422' HyperIoTDuplicateEntityException
        Alarm alarm = createAlarm();
